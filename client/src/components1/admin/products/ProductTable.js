...
const AllProduct = (props) => {
    ...
    const deleteProductReq = async (cId)=> {
      let deleteC = await deleteProduct(cId);
      if(deleteC.error){
    ...
    const editProduct = (cId,type,des,status)=> {
      if(type){
        dispatch({type:"editProductModalOpen",cId:cId,des:des,status:status})
      }
    }
          ...
          <tbody>
            {
              ...
              ? categories.map((item,key)=> {
                return (
                  <ProductTable category={item} editCat={(cId,type,des,status)=> editProduct(cId,type,des,status)} deleteCat={cId=> deleteProductReq(cId)} key={key} />
                )
              })
...
const ProductTable = ({ category, deleteCat, editCat }) => {
