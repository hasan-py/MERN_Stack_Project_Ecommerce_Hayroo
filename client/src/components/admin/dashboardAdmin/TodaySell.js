...
const SellTable = () => {
    ...
    return (
        ...
        <div className="col-span-1 overflow-auto bg-white shadow-lg p-4">
          <div className="text-2xl font-semibold mb-6 text-center">Today's Orders {data.totalOrders.Orders !== undefined  ? ordersList().length : 0}</div>
          ...
          <div className="text-sm text-gray-600 mt-2">Total {data.totalOrders.Orders !== undefined  ? ordersList().length : 0} orders found</div>
          <div className="flex justify-center">
            <span onClick={e=> history.push('/admin/dashboard/orders')} style={{background: '#303031'}}  className="cursor-pointer px-4 py-2 text-white rounded-full">View All</span>
          </div>
        </div>
      </Fragment>
    )
